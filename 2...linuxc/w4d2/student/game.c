...
#define WIN 1500
#define CENTER_X MAP_WIDTH / 2 + DISTANCE_X - 5
#define CENTER_Y MAP_HEIGHT / 2 + DISTANCE_Y
...
int ufail_k()
{
    printf("\033[%d;%dH", CENTER_Y, CENTER_X);
    printf("BAD END");
    restore_kbhit();     //终端还原
    printf("\033[?25h"); //显示光标
    fflush(stdout);
...
//     char sl = 0;
//     printf("请问要存储还是读取存档\n");
//     printf("\033[%d;%dH", CENTER_Y + 1, CENTER_X);
//     printf("       s       l      \n");
//     // int flags = fcntl(STDIN_FILENO, F_SETFD, 0);
...
// void save_game()
// {
//     FILE *sagame = fopen(SAFILE, "w");
//     if(!SAFILE) {printf("存储失败"); return;}
//     //保存食物
//     fprintf(sagame, "%d\n", food.x);
//     fprintf(sagame, "%d\n", food.x);
//     //保存蛇
...
// void load_game()
// {
//     FILE *sagame = fopen(SAFILE, "r");
//     if(!SAFILE) {printf("读取失败"); return;}
//     //读取长度
//     fscanf(sagame, "%d\n", &snake.snakelength);
//     //读取食物
//     fscanf(sagame, "%d\n", &food.x);
...
void init_termios()
{
    ...
    // fcntl(STDIN_FILENO, F_SETFL, flags | O_NONBLOCK); // 吧当前文件状态的阻塞格改成非阻塞模式，并设置（F_SETFL）
}
// 恢复终端状态
void restore_kbhit()
{
