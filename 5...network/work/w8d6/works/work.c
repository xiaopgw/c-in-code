...
void *coil(void *arg)
{
    ...
    uint16_t arr[N];
    int type, state;
    uint8_t statesla, statesla2[2];
    while (1)
    {
        // 从共享内存取出修改的开关，信号灯3P操作
        sem_op(semid, sem3, P);
        type = shmdata->coil;
        state = shmdata->value;
        ...
        // 读取开关
        modbus_read_bits(modb, 0, 1, &statesla);
        shmdata->led = statesla & 1;
        modbus_read_bits(modb, 1, 1, &statesla);
        shmdata->buz = statesla & 1;
        pthread_mutex_unlock(&lock);
        // 信号灯4V操作
        sem_op(semid, sem4, V);
    }
...
int semWebSevser(key_t key)
{
    ...
    union semun sem;
    sem.val = 0;
    int val = semctl(semid, 0, SETVAL, sem);
    // printf("sem3 initial value: %d\n", val);
...
void sem_op(int semid, int num, int op)
{
    ...
    {
        perror("semop failed");
        printf("errno = %d, semid = %d, sem_num = %d\n", errno, semid, num);
    }
}
